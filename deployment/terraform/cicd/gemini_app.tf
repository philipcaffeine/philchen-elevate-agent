...
resource "google_discovery_engine_chat_engine" "hr_gemini_app" {
  ...
  data_store_ids = [google_discovery_engine_data_store.hr_policy_datastore[each.key].data_store_id]
...
resource "null_resource" "publish_agent_to_gemini_app" {
  for_each = local.deploy_project_ids
  triggers = {
    agent_runtime_id = google_vertex_ai_reasoning_engine.app[each.key].name
    gemini_app_id    = google_discovery_engine_chat_engine.hr_gemini_app[each.key].name
  }
...
project_num = subprocess.check_output(['gcloud', 'projects', 'describe', '${each.value}', '--format=value(projectNumber)']).decode().strip()
...
headers = {
    ...
    'x-goog-user-project': '${each.value}'
}
payload = {
    ...
    'adk_agent_definition': {
        ...
        'provisioned_reasoning_engine': {'reasoning_engine': '${google_vertex_ai_reasoning_engine.app[each.key].name}'}
    }
}
